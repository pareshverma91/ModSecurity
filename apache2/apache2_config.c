...
static const char *cmd_pcre_match_timeout(cmd_parms *cmd,
        void *_dcfg, const char *p1)
{
    long val;

    if (cmd->server->is_virtual) {
        return "ModSecurity: SecPcreMatchTimeout not allowed in VirtualHost";
    }
    ...
    msc_pcre_match_timeout = (unsigned long int)val;
...
const command_rec module_directives[] = {
    ...
    AP_INIT_TAKE1 (
        "SecPcreMatchTimeout",
        cmd_pcre_match_timeout,
        NULL,
        CMD_SCOPE_MAIN,
        "PCRE match timeout limit"
    ),    
